...
import { localRevitPluginDevelopmentMashhad } from "@/content/local-revit-plugin-development-mashhad";
...
const PATH = "/revit-plugin-development-mashhad";
...
export async function generateMetadata({
  ...
  const c = localRevitPluginDevelopmentMashhad[locale as Locale] ?? localRevitPluginDevelopmentMashhad.fa;
...
export default async function RevitPluginDevelopmentMashhadPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const c = localRevitPluginDevelopmentMashhad[locale];
          ...
          <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-3">
            {p.products.map((product) => (
              <div key={product.href} className="rounded-md border border-smoke-border bg-smoke p-1">
                <div className="h-full rounded-[4px] bg-ink p-1">
                  <ProductCard
                    locale={locale}
